...
impl CharacterCategory {
    ...
    ///
    /// On WASM targets, this ignores the path and uses the embedded char.def.
    /// On native targets, this reads from the actual file path.
    #[cfg(target_arch = "wasm32")]
    pub fn from_file(_path: &Path) -> SudachiResult<CharacterCategory> {
        // WASM: always use embedded char.def (no filesystem access)
        Self::from_bytes(include_bytes!("../../../resources/char.def"))
    }
