...
#[cfg(all(feature = "native-plugins", any(target_os = "linux", target_os = "freebsd")))]
...
impl<'a, 'b, T: PluginCategory + ?Sized> PluginLoader<'a, 'b, T> {
    ...
    fn load_plugin_from_dso(
        ...
        Err(SudachiError::PluginError(PluginError::InvalidDataFormat("Loading plugin from DSO not supported without native-plugins feature".to_string())))
    }
}
