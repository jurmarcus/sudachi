...
#[cfg(feature = "sudachi")]
use crate::sudachi::{SudachiMode, SudachiTokenizer};
use crate::{
            ...
            "sudachi" | "japanese_sudachi" => {
                let mode = if value["mode"].is_null() {
                    ...
                    let mode_str = value["mode"].as_str().ok_or_else(|| {
                        anyhow::anyhow!("sudachi tokenizer 'mode' must be a string (a, b, c, or search)")
                    })?;
    ...
    pub fn to_tantivy_tokenizer(&self) -> Option<tantivy::tokenizer::TextAnalyzer> {
        let analyzer = match self {
            ...
            SearchTokenizer::Sudachi {
                ...
                add_filters!(SudachiTokenizer::new(*mode, *normalized), filters)
            }
