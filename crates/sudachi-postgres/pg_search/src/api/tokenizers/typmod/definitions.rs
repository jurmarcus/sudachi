...
pub struct SudachiTypmod {
    pub mode: tokenizers::sudachi::SudachiMode,
    pub normalized: bool,
    pub filters: SearchTokenizerFilters,
}
...
#[cfg(feature = "sudachi")]
impl TypmodRules for SudachiTypmod {
    fn rules() -> Vec<PropertyRule> {
        vec![
            rule!(
                "mode",
                ValueConstraint::StringChoice(vec!["a", "b", "c", "search"]),
                positional = 0
            ),
            rule!("normalized", ValueConstraint::Boolean),
        ]
    }
}
...
#[cfg(feature = "sudachi")]
impl TryFrom<i32> for SudachiTypmod {
    type Error = typmod::Error;
        ...
        let mode = parsed
            ...
            .map(|p| match p.as_str() {
                Some(s) => {
                    let lcase = s.to_lowercase();
                    match lcase.as_str() {
                        "a" => tokenizers::sudachi::SudachiMode::A,
                        "b" => tokenizers::sudachi::SudachiMode::B,
                        "c" => tokenizers::sudachi::SudachiMode::C,
                        "search" => tokenizers::sudachi::SudachiMode::Search,
                        other => panic!("unknown sudachi mode: {other}"),
                    }
                }
                None => tokenizers::sudachi::SudachiMode::Search,
            })
            .unwrap_or(tokenizers::sudachi::SudachiMode::Search);
