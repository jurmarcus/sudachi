//! `FilterMisparse` — Filter morpheme sequences known to be misparses (logged-but-discarded).
//!
...
//! in a follow-up commit. Search the C# source for `FilterMisparse` to find
...
/// the [`Phase::Cleanup`] phase, and the [`MorphemeFeatures`]
/// gate.
pub fn stage() -> Stage {
    Stage::new(NAME, Phase::Cleanup, MorphemeFeatures::empty(), apply)
}
