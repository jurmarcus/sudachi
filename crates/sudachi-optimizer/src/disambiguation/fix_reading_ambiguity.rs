//! `FixReadingAmbiguity` — Resolve reading ambiguity using neighbouring morphemes (行く: いく vs ゆく).
//!
...
//! in a follow-up commit. Search the C# source for `FixReadingAmbiguity` to find
...
/// the [`Phase::Disambiguation`] phase, and the [`MorphemeFeatures`]
/// gate.
pub fn stage() -> Stage {
    Stage::new(NAME, Phase::Disambiguation, MorphemeFeatures::empty(), apply)
}
