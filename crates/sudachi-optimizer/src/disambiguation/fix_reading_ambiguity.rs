//! `FixReadingAmbiguity` — Resolve reading ambiguity using neighbouring tokens (行く: いく vs ゆく).
//!
...
//!
//! Method: `FixReadingAmbiguity` (search the C# source for the function definition).
...
/// the [`StageGroup::Disambiguation`] grouping, and the [`TokenFeatures`]
/// gate.
pub fn stage() -> Stage {
    Stage::new(NAME, StageGroup::Disambiguation, TokenFeatures::empty(), apply)
}
