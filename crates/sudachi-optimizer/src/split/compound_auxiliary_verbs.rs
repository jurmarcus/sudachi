//! `SplitCompoundAuxiliaryVerbs` — Split compound verbs Sudachi outputs as one token (e.g., し終わっ → し + 終わっ).
//!
...
//!
//! Method: `SplitCompoundAuxiliaryVerbs` (search the C# source for the function definition).
...
/// the [`StageGroup::Split`] grouping, and the [`TokenFeatures`]
/// gate.
pub fn stage() -> Stage {
    Stage::new(NAME, StageGroup::Split, TokenFeatures::empty(), apply)
}
